...
int i = 0;
mlib::vec3 translate(0.0f), rotate({ 0.0f, 90.0f, 0.0f }), scale(0.7f);
mlib::mat4 model = mlib::scale(scale).matmul(mlib::translate(translate));
/*
    Why you don't see the textured quad at first [for mlib::vec3 rotate(0.0f)]:
    The mlib::rotate cancels out the x component of the matrix.
...
int main()
{
    ...
    //VAO vertexArray = VAO::Create();
    /*
        VAO buffer = {data, { attribute0 }, { atrribute1 }, { attribute2 } }
        Each attribute is made up of: vec Length, normilization bool, GLenum data_type, offset and stride.
    */
...
void processInput(GLFWwindow* window) {
    ...
    else if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS)     rotate.x += (float)mlib::PI * dt;
    else if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)     rotate.x -= (float)mlib::PI * dt;
    else if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS)     rotate.y += (float)mlib::PI * dt;
    else if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS)     rotate.y -= (float)mlib::PI * dt;
    else if (glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS)     rotate.z += (float)mlib::PI * dt;
    else if (glfwGetKey(window, GLFW_KEY_6) == GLFW_PRESS)     rotate.z -= (float)mlib::PI * dt;
    t0 = t1;
    model = mlib::scale(scale).matmul(mlib::translate(translate).matmul(mlib::rotate(rotate.x, rotate.y, rotate.z)));
    std::string msg = model.str();
    if (i % 100 == 0) {
        std::cout << msg;
    } if (i % 101 == 0) ClearScreen();
    i++;
}
